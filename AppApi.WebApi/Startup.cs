...
namespace AppApi.WebApi
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            //        .AddValidatorsFromAssemblyContaining<EmailConfigProfile>();
            ...
            //    services.AddAutoMapper(typeof(EmailConfigProfile).Assembly);
            ...
            services
            .InitCors()
            .InitSqlServer<WebApiDbContext>(Configuration, "WebApiConnection")
            // .InitJWT()
            ...
            .InitMapping()
            .InitUnitOfWorkAndServices("WebApi")
            .InitAutoMapper()
            .InitWebApiOpeniddict<WebApiDbContext>(Configuration)
            ;

            // services.AddSingleton<IHostedService>(serviceProvider =>
            // {
            //     var logger = serviceProvider.GetRequiredService<ILogger<T>>();
            //     return (IHostedService)Activator.CreateInstance(typeof(T),
            //         config.CronExpression, config.TimeZoneInfo, serviceProvider, logger);
            // });

            // services.AddCronJob<CronJobUpdateDonViDeNghiService>(c =>
            //{
            //    c.TimeZoneInfo = TimeZoneInfo.Local;
            // //    c.CronExpression = @"*/15 * * * *";
            //    c.CronExpression = @"* * * * *";
            ...
            //services.AddSingleton<IScheduleConfig<CronJobUpdateDonViDeNghiService>>(new ScheduleConfig<CronJobUpdateDonViDeNghiService>
            //{
            //    CronExpression = "* * * * *",
            ...
            services.AddSingleton<IHostedService, CronJobUpdateDonViDeNghiService>(sp =>
            {
                var config = sp.GetRequiredService<IScheduleConfig<CronJobUpdateDonViDeNghiService>>();
                var logger = sp.GetRequiredService<ILogger<CronJobUpdateDonViDeNghiService>>();
                return new CronJobUpdateDonViDeNghiService(sp, config, logger);
            });
        }
        ...
        {
            app.UseAllMiddlewares<WebApiDbContext>(env, provider, typeof(WebApi.Controllers.FileManagerController).Assembly,
                    "AppApi.WebApi.Controllers", false, true);
